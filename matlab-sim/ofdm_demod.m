function [decoded_symb, decoded_phase] = ofdm_demod(symb_rx, ifft_size, carriers, conj_carriers, guard_time, symb_size, word_size, last, unpad, fig)
    ...
    symb_rx_matrix = reshape(symb_rx(1:symb_period*floor(length(symb_rx)/symb_period)), symb_period, floor(length(symb_rx)/symb_period));
    ...
    if fig == 1
        limt = 1.1 * max(abs(reshape(rx_spectrum_matrix', 1, size(rx_spectrum_matrix, 1) * size(rx_spectrum_matrix, 2))));
        figure(5)
        stem(0:ifft_size-1, abs(rx_spectrum_matrix(ceil(size(rx_spectrum_matrix, 1)/2), 1:ifft_size)), 'b*-')
        grid on
        ...
        figure(6)
        plot(0:ifft_size-1, (180/pi) * angle(rx_spectrum_matrix(ceil(size(rx_spectrum_matrix, 1)/2), 1:ifft_size)), 'go')
        hold on
        stem(carriers-1, (180/pi) * angle(rx_spectrum_matrix(2, carriers)), 'b*-')
        stem(conj_carriers-1, (180/pi) * angle(rx_spectrum_matrix(ceil(size(rx_spectrum_matrix, 1)/2), conj_carriers)), 'b*-')
        axis([0 ifft_size -200 +200])
        grid on
        ylabel('Phase (degrees)')
        xlabel('FFT Bin')
        title('Phase of Received OFDM Spectrum')
    end
    ...
    if fig == 1
        ...
        polar(rx_phase(ceil(size(rx_spectrum_matrix, 1)/2), :) * (pi/180), rx_mag, 'bd')
    ...
    % Parallel to serial conversion of phases
    ...
    decoded_phase = rem(decoded_phase/base_phase + 0.5, (2^symb_size)) * base_phase - 0.5 * base_phase;
